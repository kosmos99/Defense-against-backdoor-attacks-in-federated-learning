...
class FixedPoolFederatedLearningTrainer(FederatedLearningTrainer):
    ...
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        if arguments["defense_technique"] == "no-defense":
            ...
            self._defender = RFA()
        elif arguments["defense_technique"] == "soft_hard":
            num_adv = int(self.attacker_percent*self.part_nets_per_round)
            self._defender = SoftHardFiltering(total_workers=self.num_nets, num_workers=self.part_nets_per_round, num_adv=num_adv, num_valid=1, instance=arguments['instance'], use_trustworthy=self.use_trustworthy)
    ...
    def run(self, wandb_ins=None):
        ...
        for flr in range(1, self.fl_round+1):
            ...
            elif self.defense_technique == "soft_hard":
